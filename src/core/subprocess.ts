...
export function safeGitCommand(args: string[], options: SpawnSyncOptions = {}) {
    ...
    if (args.length === 0 || !allowedGitCommands.includes(args[0])) {
        throw new Error(`Git command not allowed: ${args[0] ?? '(empty)'}`);
    }
...
export function safeDockerCommand(args: string[], options: SpawnSyncOptions = {}) {
    ...
    if (args.length === 0 || !allowedDockerCommands.includes(args[0])) {
        throw new Error(`Docker command not allowed: ${args[0] ?? '(empty)'}`);
    }
